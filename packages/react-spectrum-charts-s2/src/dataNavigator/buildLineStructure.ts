...
import { DEFAULT_TIME_DIMENSION, NAVIGATION_INDEX_FIELD, SINGLE_LINE_NODE_ID } from '@spectrum-charts/constants';
...
/**
 * Left/Right always stay within the current line; Up/Down jump to the point at the same position
 * in the adjacent line while drilled into a point, or to the adjacent line itself while at the line level.
 */
...
const groupIntoLines = (data: SimpleData[], color: string | undefined): Line[] => {
  ...
  for (const row of data) {
    ...
    const mapKey = key ?? SINGLE_LINE_NODE_ID;
    let line = lineByKey.get(mapKey);
    if (!line) {
      line = { key, divisionId: color ? `line_${mapKey}` : SINGLE_LINE_NODE_ID, pointIds: [], rows: [] };
...
export const buildLineStructure = ({
  ...
  lines.forEach((line, i) => {
    ...
    if (line.pointIds.length) {
      ...
      addOneSidedEdge(edges, nodes, line.divisionId, line.pointIds[line.pointIds.length - 1], line.divisionId, ['left']);
    }
...
export const buildNodeLabel = (
  ...
  const parts = Object.entries(data)
    .filter(
      ([key, value]) =>
        key !== NAVIGATION_INDEX_FIELD && !key.startsWith('_') && value != null && typeof value !== 'object' && typeof value !== 'function'
    )
    .map(([key, value]) => {
      const label = fieldLabels[key] ?? key;
      const displayValue = key === dimension && isTimeDimension ? formatDateValue(value, locale) : value;
