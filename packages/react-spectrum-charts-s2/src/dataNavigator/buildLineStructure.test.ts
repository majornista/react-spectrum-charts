...
/** Follows a single edge-tagged move manually, mirroring how data-navigator's own input resolves one. */
const move = (structure: Structure, from: string, navId: string): string | undefined => {
...
describe('getLineNodeId()', () => {
  ...
  test('keys a single-line datum by its index alone', () => {
    expect(getLineNodeId({ datetime: 0, [NAVIGATION_INDEX_FIELD]: 2 })).toBe('2');
  });

  test('keys a multi-line datum by the series + index composite', () => {
    expect(getLineNodeId({ datetime: 0, series: 'A', [NAVIGATION_INDEX_FIELD]: 2 }, 'series')).toBe(segmentId('A', 2));
  });
});
...
describe('buildNodeLabel()', () => {
  ...
  test('describes a leaf node by its scalar fields, excluding the injected nav-index field', () => {
    const node = { id: '1', data: { datetime: 0, value: 28, [NAVIGATION_INDEX_FIELD]: 1 } } as unknown as NodeObject;
    const label = buildNodeLabel(node, 'datetime', false);
    expect(label).toContain('datetime: 0');
    expect(label).toContain('value: 28');
    expect(label).not.toContain(NAVIGATION_INDEX_FIELD);
  });
